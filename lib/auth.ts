...
const JWT_SECRET = new TextEncoder().encode(process.env.NEXTAUTH_SECRET || 'pluginguard-ai-proactive-wordpress-plugin-security-audits-secret-2026');

async function hashPw(pw: string): Promise<string> {
  const data = new TextEncoder().encode(pw + 'plu-salt');
...
export async function auth(req?: NextRequest) {
  try {
    ...
    if (req) { token = req.cookies.get('plu_session')?.value; }
    else { const { cookies } = await import('next/headers'); const cs = await cookies(); token = cs.get('plu_session')?.value; }
